...
class OpenAIAssistantScanner:
    ...
    def should_scan_assistant(self, assistant_id: str, modified_at: int) -> bool:
        ...
        return self.scanned_assistants[assistant_id].get('modified_at', 0) != modified_at
    ...
    def scan_assistant_instructions(self, assistant: Dict) -> Optional[Dict]:
        ...
        if not instructions:
            print(f"Assistant {assistant_name} ({assistant_id}) has no instructions to scan")
            return None
        ...
        try:
            documents = [{
                ...
                "filename": f"openai_assistant_{assistant_id}_instructions.md"
            }]
            ...
            if hasattr(multi_scan_result, 'scan_results') and multi_scan_result.scan_results:
                ...
                print(f"Error scanning assistant {assistant_name}: {multi_scan_result}")
                return None
    ...
    def process_scan_results(self, scan_data: Dict) -> int:
        ...
        print(f"⚠️  Found {scan_result.policy_break_count} potential secret(s) in {assistant_name}")
    ...
    def run_scan(self):
        ...
        if self.force_rescan:
            print("🔄 Force rescan mode: Will rescan all assistants regardless of modification time")
        print()
        ...
        if assistants is None:
            print("❌ Failed to fetch assistants. Please check your OpenAI API key and connection.")
            return
        if not assistants:
        ...
        scanned_count = skipped_count = no_instructions_count = secrets_found = 0
        
        for assistant in assistants:
            assistant_id = assistant.get('id')
            assistant_name = assistant.get('name', 'Unnamed Assistant')
            modified_at = assistant.get('modified_at', assistant.get('created_at', 0))
