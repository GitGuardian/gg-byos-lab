...
Scans GitLab snippets for a specific user or authenticated user's snippets
for secrets and sensitive information using GitGuardian's BYOS
(Bring Your Own Source) functionality.

Supports both GitLab.com (SaaS) and self-hosted GitLab instances.
Tracks scanned snippets by update timestamp to avoid rescanning
unchanged snippets.
"""
...
class GitLabSnippetScanner:
    def __init__(self):
        ...
        if missing:
            ...
            print(f"\nAll required: {', '.join(required_env_vars.keys())}")
            sys.exit(1)

        # GitLab configuration
        self.gitlab_base_url = os.getenv(
            "GITLAB_BASE_URL", "https://gitlab.com"
        ).rstrip("/")

        # Configure GitGuardian client with increased timeout
        self.gg_client = GGClient(
            api_key=self.gitguardian_api_key, timeout=60
        )
    ...
    def should_scan_snippet(self, snippet_id: str, updated_at: str) -> bool:
        ...
        return (
            self.scanned_snippets[snippet_key].get("updated_at") != updated_at
        )
    ...
    def get_authenticated_user_snippets(self) -> Optional[List[Dict]]:
        ...
        try:
            ...
            while True:
                ...
                # GitLab API returns fewer results than per_page
                # when on last page
    ...
    def _enhance_snippet_info(self, snippets: List[Dict]) -> List[Dict]:
        ...
        for snippet in snippets:
            if self.verbose:
                print(
                    f"🔍 Snippet data for '{snippet.get('title', 'No title')}':"
                )
            ...
            # The key difference:
            #   project snippets have path components before /-/snippets/
            is_project_snippet = (
                snippet.get("project_id") is not None
                or snippet.get("project") is not None
            )
            ...
            if not is_project_snippet and "/-/snippets/" in web_url:
                ...
                if len(url_parts) > 0:
                    ...
                    if "://" in before_snippets:
                        path_part = "/".join(
                            before_snippets.split("/")[3:]
                        )  # Skip protocol, domain
                    else:
                        path_part = before_snippets

                    # If there's a path before /-/snippets/:
                    #   it's a project snippet
                    # User snippets:
                    #   https://gitlab.com/-/snippets/123 -> path_part = ""
                    # Project snippets:
                    #   https://gitlab.com/group/project/-/snippets/123
                    #   -> path_part = "group/project"
            ...
            if is_project_snippet:
                ...
                if project_info and isinstance(project_info, dict):
                    snippet["_project_name"] = project_info.get(
                        "name", "Unknown Project"
                    )
                    snippet["_project_path"] = project_info.get(
                        "path_with_namespace", project_info.get("path", "")
                    )
                elif "/-/snippets/" in web_url:
                    # Extract project path from URL like:
                    #   https://gitlab.com/group/project/-/snippets/123
                    url_parts = web_url.split("/-/snippets/")
                    if len(url_parts) > 0:
                        # Get everything before /-/snippets/
                        # and extract the project part
                        project_url = url_parts[0]
                        # Remove the base GitLab URL to get
                ...
                if self.verbose:
                    print(
                        "✅ Detected project snippet:"
                        f" {snippet.get('_project_name', 'Unknown')}"
                    )
            else:
                snippet["_snippet_type"] = "user"
                if self.verbose:
                    print("👤 Detected user snippet")

        return snippets

    def get_gitlab_snippets(self) -> Optional[List[Dict]]:
        """
        Fetch snippets from GitLab API for authenticated user.
    ...
    def get_snippet_details(
        self, snippet_id: str, project_id: Optional[str] = None
    ) -> Optional[Dict]:
        ...
        if project_id:
            url = (
                ...
                f"/api/v4/projects/{project_id}/snippets/{snippet_id}"
            )
        else:
    ...
    def get_snippet_raw_content(
        ...
        if project_id:
            url = (
                f"{self.gitlab_base_url}"
                f"/api/v4/projects/{project_id}/snippets/{snippet_id}/raw"
            )
        else:
        ...
        try:
            ...
            print(
                f"❌ Error fetching snippet raw content for {snippet_id}: {e}"
            )
            return None
    ...
    def scan_snippet_content(self, snippet: Dict) -> Optional[Dict]:
        ...
        if not content.strip():
            print(
                f"Snippet {snippet_id} ({snippet_title})"
        ...
        if project_name:
            print(
                f"Scanning snippet:"
                f" {snippet_title or snippet_id} (Project: {project_name})"
            )
        else:
        ...
        try:
            ...
            username = (
                author.get("username", "authenticated_user")
                if author
            ...
            project_info = (
                f"_project_{snippet.get('_project_id')}"
                if snippet_type == "project"
                else ""
            )
            ...
            documents = [
                {
                    "document": content,
                    "filename": (
                        f"gitlab_{snippet_type}_snippet_{username}"
                        f"{project_info}_{snippet_id}_{snippet_filename}"
                    ),
                }
            ]
            ...
            if (
                ...
                print(
                    f"Error scanning snippet {snippet_id}: {multi_scan_result}"
                )
                return None
    ...
    def process_scan_results(self, scan_data: Dict) -> int:
        ...
        snippet_display = (
            f"{snippet_title} (Project: {project_name})"
            if project_name
            else snippet_title
        )
        ...
        print(
            ...
            f" secret(s) in snippet: {snippet_display}"
        )
    ...
    def run_scan(self):
        ...
        print("📋 Starting GitLab Snippet Scanner for authenticated user")
        print(f"GitLab Instance: {self.gitlab_base_url}")
        print(f"Source UUID: {self.source_uuid}")
        if self.force_rescan:
            print(
                "🔄 Force rescan mode: Will rescan all snippets"
                " regardless of update timestamp"
            )
        print()
        ...
        if snippets is None:
            print(
                ...
                " Please check your GitLab API key and permissions."
            )
            sys.exit(1)
        if not snippets:
            print("ℹ️  No snippets found for specified target(s).")
            return
        ...
        for snippet in snippets:
            ...
            if scan_data:
                ...
                self.scanned_snippets[snippet_id] = {
                    "title": snippet_title,
                    "filename": snippet.get(
                        "file_name",
                        f"snippet_{snippet_id}"
                    ),
