...
class DustAgentScanner:
    ...
    def scan_agent_instructions(self, agent: Dict) -> Optional[Dict]:
        ...
        if not instructions:
            print(f"Agent {agent_name} ({agent_id}) has no instructions to scan")
            return None
        
        print(f"Scanning agent: {agent_name} ({agent_id})")
        
        try:
            documents = [{"document": instructions, "filename": f"dust_agent_{agent_id}_instructions.md"}]
            multi_scan_result = self.gg_client.scan_and_create_incidents(documents, source_uuid=self.source_uuid)
            ...
            if hasattr(multi_scan_result, 'scan_results') and multi_scan_result.scan_results:
                ...
                print(f"Error scanning agent {agent_name}: {multi_scan_result}")
                return None
    ...
    def process_scan_results(self, scan_data: Dict) -> int:
        ...
        agent_name, scan_result = scan_data['agent_name'], scan_data['scan_result']
        
        if not (hasattr(scan_result, 'policy_break_count') and scan_result.policy_break_count > 0):
            print(f"✅ No secrets found in {agent_name}")
            return 0
        
        print(f"⚠️  Found {scan_result.policy_break_count} potential secret(s) in {agent_name}")
    ...
    def run_scan(self):
        ...
        if self.force_rescan:
            print("🔄 Force rescan mode: Will rescan all agents regardless of version")
        print()
        ...
        for agent in agents:
            agent_id, agent_name, version = agent.get('sId'), agent.get('name', 'Unknown'), agent.get('version', 0)
            ...
            if scan_data:
                ...
                self.scanned_agents[agent_id] = {
                    ...
                    'last_scanned': datetime.now().isoformat(), 'source_uuid': self.source_uuid
                }
                scanned_count += 1
            print()
