...
Scans Claude project configurations and instructions for secrets
and sensitive information using GitGuardian's BYOS (Bring Your Own Source)
functionality.

Tracks scanned projects by configuration hash to avoid rescanning unchanged
projects.
"""
...
class ClaudeProjectScanner:
    ...
    def should_scan_project(
        ...
        return (
            self.scanned_projects[project_name].get("content_hash")
            != content_hash
        )
    ...
    def scan_project_content(self, project: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if (
                ...
                print(
                    f"Error scanning project {project_name}:"
                    f" {multi_scan_result}"
                )
                return None
    ...
    def process_scan_results(self, scan_data: Dict) -> int:
        ...
        print(
            ...
            f" potential secret(s) in {project_name}"
        )
    ...
    def run_scan(self):
        ...
        if self.force_rescan:
            print(
                "🔄 Force rescan mode:"
                " Will rescan all projects regardless of content hash"
            )
        print()
        ...
        if not projects:
            print(
                "No projects found."
                " Please add project files to the projects directory."
            )
            return
